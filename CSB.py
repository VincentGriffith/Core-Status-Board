...
class Outside: #contains and draws elements not in core or racks
    def __init__(self):
        self.outs = []
    def add(self,new):
...
class Element:
    ...
    def remove(self):
        ...
        self.insert(None)
    def highlight(self):
        pass #will change later to visually indicate selection
    def unhighlight(self):
        pass #will change later to remove highlight
...
class Position:
    def __init__(self,x,y,r,name):
        ...
        self.name = "name"
        self.contains = None
        mycanvas.tag_bind(self.circle,'<Button-1>',self.click)
...
def select(event):
    pass
    #coming back to this code later, I'm honestly not sure what I inteded for this, but I'm leaving it as a possible reminder if I remember my original intention
...
def move(first,second):
    if type(first) is Position and type(second) is Position:
        clear_selections() #clear selections, input is nonsense
        print('your test is working')
    elif type(first) is Element and type(second) is Position:
        first.position.contains = None
        first.position = second
        second.contains = first #move first into second
    elif type(first) is Position and type(second) is Element:
        move(second,first) #selections are backwards
    else: #two elements selected, switch them
        first.position.contains = second
        second.position.contains = first
        first.position, second.position = second.position, first.position #switch position pointers
...
elemenu.add_command(label = "Move", command = move_mode)
elemenu.add_command(label = "Delete", command = del_mode)
